defmodule MemoryTest do
  ...
  test "write and read" do
    ...
    vals = Enum.map(0..2047, fn address -> ({_,v} = Memory.read(m, address); v) end)
    ...
    m = Enum.reduce(0..2047, m, fn address, mem -> ({m1, _} = Memory.read(mem, address); m1) end)
  ...
  test "write sequential" do
    ...
    vals = Enum.map(address_range, fn address -> ({_,v} = Memory.read(m, address); v) end)
    assert m.w_count == count
    assert Enum.sum(vals) == div(count * 2 * (count-1), 2) # I know, it doesn't need the div if no "* 2"
  end
end
